...
export const LiveInterface: React.FC<LiveInterfaceProps> = ({ onClose }) => {
  ...
  >([]);
  const [arHuman, setArHuman] = useState<{
    type: string;
    x: number;
    y: number;
  } | null>(null);
  ...
  const [conversationState, setConversationState] = useState("assessment");
  ...
  const [specialtyMode, setSpecialtyMode] = useState<
    "CLINICAL" | "SURGICAL" | "LAB" | "BIOMEDICAL" | "RADIOLOGY"
  >("CLINICAL");
  ...
  >([]);
  const [improvisedTools, setImprovisedTools] = useState<
    {
      originalItem: string;
      medicalUse: string;
      instructions: string;
  ...
  const [spatialMap, setSpatialMap] = useState<
    { id: string; x: number; y: number; level: string; timestamp: number }[]
  >([]);
  const [anatomyOverlay, setAnatomyOverlay] = useState<
    "skeleton" | "circulatory" | "respiratory" | "nervous" | null
  >(null);
  const [hapticPattern, setHapticPattern] = useState<
    "cpr" | "pressure" | "steady" | "rhythmic_breathing" | null
  >(null);
  const [showMSR, setShowMSR] = useState(false);
  const [showRadiologyTrainer, setShowRadiologyTrainer] = useState(
  ...
  >("low");
  const [activeEmergencyType, setActiveEmergencyType] =
  ...
  const [multiSpectralAnalysis, setMultiSpectralAnalysis] = useState<{
    hypoxiaDetection: number;
    internalBleedingProbability: number;
    tissueViability: number;
  } | null>(null);
  const [woundMetrics, setWoundMetrics] = useState<{
    lengthMm: number;
    depthMm: number;
  ...
  const [radiologyWorkflowState, setRadiologyWorkflowState] = useState<
    | "IDLE"
    | "ASKING_SCAN_TYPE"
    | "PROVIDING_GUIDANCE"
    | "AWAITING_SCAN"
  ...
  const protocolName = "LIVE EMERGENCY GUIDE";
  ...
  const startRadiologyWorkflow = async () => {
    ...
    triggerHaptic("info");
    speak(
      "What type of radiology scan would you like to perform today? For example, Brain MRI or Chest CT.",
    );
  };
  ...
  const handleRadiologyVocalInput = async (text: string) => {
    if (radiologyWorkflowState === "ASKING_SCAN_TYPE") {
      ...
      try {
        ...
        speak(
          "Device guidance is ready. Follow the instructions on your screen to prepare the patient and the equipment.",
        );
      } catch (error) {
        console.error("Error getting guidance:", error);
        speak(
          "I encountered an error retrieving device guidance. Please try again.",
        );
      }
    }
  };
  ...
  const speak = async (text: string) => {
    ...
    const now = Date.now();
    if (
      normalizedText === lastSpokenTextRef.current &&
  ...
  const startLiveMode = async () => {
    try {
      ...
      setStatus('Ready: tell me what is happening');
  ...
  const setupSpeechRecognition = () => {
    ...
    recognition.onerror = (event: any) => {
      ...
      if (event.error === "aborted") {
        console.log(
          "Speech recognition aborted - this is usually normal during mode switches",
        );
        return;
      }
  ...
  const handleUserSpeech = (text: string) => {
    ...
    if (
      lowerText === lastUserSpeechRef.current.text &&
      now - lastUserSpeechRef.current.at < 1500
    ) {
      return;
    }
    lastUserSpeechRef.current = { text: lowerText, at: now };

    setChatHistory((prev) => [...prev, { role: "user", content: text }]);
    setConversationState("assessment");

    if (
      specialtyMode === "RADIOLOGY" &&
    ...
    const confirmsAirMoving =
      /breathing|breathing normally|can talk|able to talk|talking|responsive|awake|conscious/.test(
        lowerText,
      ) && !/cannot|can't|not/.test(lowerText);
    const confirmsConscious =
      /awake|alert|responsive|conscious|talking|can answer/.test(lowerText) &&
      !/unconscious|not responsive|not conscious/.test(lowerText);
    const confirmsUnresponsive =
      /unresponsive|unconscious|not responding|collapsed|limp|no pulse|not breathing/.test(
        lowerText,
      );

    if (confirmsAirMoving) {
      setVitalChecks((prev) => ({
        ...prev,
        breathing: true,
      }));
    ...
    if (confirmsUnresponsive) {
      setVitalChecks({ breathing: false, conscious: false });
      setConversationState("unresponsive");
  ...
  const startVitalScan = () => {
    ...
    setTimeout(() => {
      ...
      triggerHaptic("success");
      speak(
        `Vital scan complete. Estimated heart rate is ${hr} beats per minute.`,
      );
  ...
  const performAILoop = async (userQuery?: string) => {
    ...
    try {
      if (videoRef.current && isActive) {
        ...
        const context = `
          Protocol: general live emergency intake
          Active Emergency Type: ${activeEmergencyType}
          Active Emergency Label: ${activeEmergencyLabel}
          Conversation State: ${conversationState}
          Breathing: ${vitalChecks.breathing ? "yes" : "unknown"}
          Conscious: ${vitalChecks.conscious ? "yes" : "unknown"}
          Last Prompt: ${lastQuestion || "none"}
          Prior Response: ${lastResponse || "none"}
        `;
        ...
        if (specialtyMode === "RADIOLOGY") {
          if (
            ...
            result = await analyzeRadiology(
              base64Image,
              context,
              (window as any).isRadiologyTraining,
            );
            setRadiologyResult(result);
            setRadiologyWorkflowState("RESULTS");
            setShowRadiologyTrainer(true);
            speak(
              "Medical grade analysis complete. I have identified the findings and prepared treatment, recovery, and surgical plans as applicable.",
            );
          } else {
            setIsThinking(false);
            return;
        ...
        if (result.emergencyType) setActiveEmergencyType(result.emergencyType);
        if (result.emergencyLabel) setActiveEmergencyLabel(result.emergencyLabel);
        ...
        if (isMCIMode && result.arPoints) {
          const tags = result.arPoints
            .filter((p) =>
              ["RED", "YELLOW", "GREEN", "BLACK"].some((level) =>
                p.label.includes(level),
              ),
            )
            .map((p, i) => ({
              id: i,
              x: p.x,
              y: p.y,
              level: (p.label.match(/RED|YELLOW|GREEN|BLACK/) || [
                "GREEN",
              ])[0] as any,
            }));
          if (tags.length > 0) {
            ...
            setSpatialMap((prev) => {
              const newMap = [...prev];
              tags.forEach((tag) => {
                const existingIdx = newMap.findIndex(
                  (m) =>
                    Math.abs(m.x - tag.x) < 10 && Math.abs(m.y - tag.y) < 10,
                );
                if (existingIdx >= 0) {
                  newMap[existingIdx] = {
                    ...newMap[existingIdx],
        ...
        if (result.predictiveAnalytics)
          setPredictiveAnalytics(result.predictiveAnalytics);
        if (result.multiSpectralAnalysis)
          setMultiSpectralAnalysis(result.multiSpectralAnalysis);
        if (result.woundMetrics) setWoundMetrics(result.woundMetrics);

        setArPoints(
          result.arPoints.map((p) => ({ x: p.x, y: p.y, label: p.label })),
        );
        ...
        if (result.phase === "unresponsive") {
          setConversationState("unresponsive");
          setStatus("CPR guidance active");
        } else if (result.phase === "action") {
          setConversationState("action");
          setStatus("Action steps active");
        } else {
          setConversationState("assessment");
      ...
      if (err.message === "QUOTA_EXCEEDED") {
        ...
        setStatus("Temporary error... retrying");
      }
    } finally {
      setIsThinking(false);
      if (!userQuery && isActive && specialtyMode === "RADIOLOGY") {
  ...
  return (
    <div className="fixed inset-0 bg-black z-200 flex flex-col">
        ...
        {!isActive && (
          ...
          >
            ...
            <h2 className="text-3xl font-light tracking-tight mb-4">
              Start Live Emergency Guide
            </h2>
            <p className="text-white/40 text-sm mb-12 max-w-xs leading-relaxed">
              Tell me the emergency in plain language and I will identify the
              likely problem, ask follow-up questions when needed, and give
              step-by-step first-aid guidance.
            </p>
            <button
              onClick={() => {
                console.log("Establishing Link...");
                setActiveEmergencyType("unknown");
        ...
        {isQuotaExceeded && (
          ...
          >
            ...
            <p className="text-white/60 mb-8 max-w-xs leading-relaxed">
              The free Gemini API quota has been reached. To continue using
              Universal Live, please link a paid Google Cloud project.
            </p>
            ...
            <div className="mt-12 p-6 rounded-2xl bg-white/5 border border-white/10 text-left">
              ...
              <p className="text-[10px] text-white/60 leading-relaxed">
                Universal Live performs high-frequency visual analysis. Free
                tier keys have strict rate limits. Linking a paid project
                enables unlimited high-speed processing.
              </p>
              <a
        ...
        <div className="absolute inset-0 pointer-events-none">
          ...
          {anatomyOverlay && (
            ...
            >
              <svg
                viewBox="0 0 100 100"
                className="w-full h-full text-blue-400/30"
              >
                {anatomyOverlay === "skeleton" && (
                  <path
                    d="M50 10 L50 90 M30 30 L70 30 M30 50 L70 50 M40 70 L60 70"
                    stroke="currentColor"
                    strokeWidth="0.5"
                    fill="none"
                  />
                ...
                {anatomyOverlay === "nervous" && (
                  <path
                    d="M50 10 L50 90 M50 20 L20 40 M50 20 L80 40 M50 40 L15 70 M50 40 L85 70"
                    stroke="currentColor"
                    strokeWidth="0.2"
                    fill="none"
                    strokeDasharray="1 1"
          ...
          {predictiveAnalytics && (
            <div className="absolute top-32 left-6 w-32 p-4 bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden">
              <div className="text-[8px] font-mono text-emerald-400 uppercase tracking-widest mb-3">
                Bio-Digital Twin
              </div>
              ...
              <div className="mt-2 space-y-1">
                {predictiveAnalytics.organSystemRisk
                  .slice(0, 3)
                  .map((risk, i) => (
                    <div
                      key={i}
                      className="flex justify-between text-[6px] font-mono uppercase"
                    >
            ...
            <div className="absolute top-6 left-1/2 -translate-x-1/2 flex gap-4">
              <div className="px-4 py-2 bg-black/60 backdrop-blur-xl border border-white/10 rounded-full flex items-center gap-3">
                ...
                <span className="text-[8px] font-mono text-purple-400 uppercase tracking-widest">
                  Spectral_Hypoxia:{" "}
                  {(multiSpectralAnalysis.hypoxiaDetection * 100).toFixed(1)}%
                </span>
              </div>
              <div className="px-4 py-2 bg-black/60 backdrop-blur-xl border border-white/10 rounded-full flex items-center gap-3">
                ...
                <span className="text-[8px] font-mono text-amber-400 uppercase tracking-widest">
                  Tissue_Viability:{" "}
                  {(multiSpectralAnalysis.tissueViability * 100).toFixed(1)}%
                </span>
              </div>
            </div>
          )}
          ...
          {predictiveAnalytics && (
            <div className="absolute bottom-32 right-6 w-48 p-4 bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl">
              <div className="text-[8px] font-mono text-blue-400 uppercase tracking-widest mb-2">
                Predictive Survival Timeline
              </div>
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-[10px] font-bold text-emerald-400">
                  {predictiveAnalytics.survivalProbability}%
                </span>
              </div>
              <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden mb-3">
                <motion.div
                  initial={{ width: 0 }}
                  animate={{
                    width: `${predictiveAnalytics.survivalProbability}%`,
                  }}
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-mono text-red-400 animate-pulse">
                  {predictiveAnalytics.timeToCriticalityMin}m 00s
                </span>
              </div>
            </div>
          )}
          ...
          {isMCIMode && spatialMap.length > 0 && (
            <div className="absolute top-32 right-6 w-24 h-24 rounded-full bg-black/40 backdrop-blur-md border border-white/10 overflow-hidden">
              ...
              <div className="absolute bottom-1 left-1/2 -translate-x-1/2 text-[6px] font-mono text-white/40 uppercase">
                Scene Radar
              </div>
            </div>
          )}
            ...
            >
              <div className="group relative">
                ...
                <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-48 p-3 bg-black/80 backdrop-blur-xl border border-blue-500/30 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity">
                  <div className="text-[8px] font-mono text-blue-400 uppercase tracking-widest mb-1">
                    Improvised Tool
                  </div>
                  <div className="text-[10px] font-bold text-white mb-1">
                    {tool.originalItem} → {tool.medicalUse}
                  </div>
            ...
            >
              <div className="relative w-32 h-32">
                ...
                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-full mt-2 bg-amber-500/20 backdrop-blur-md border border-amber-500/30 px-2 py-1 rounded text-[8px] font-mono text-amber-400 whitespace-nowrap">
                  L: {woundMetrics.lengthMm}mm | D: {woundMetrics.depthMm}mm |
                  A: {woundMetrics.surfaceAreaMm2}mm²
                </div>
              </div>
            </motion.div>
          )}
              ...
              >
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: AR_HUMAN_POSES[arHuman.type].svg,
                  }}
                />
            ...
            <div className="flex items-center gap-3 px-4 py-2 bg-black/60 backdrop-blur-xl border border-white/10 rounded-full">
              <div
                className={cn(
                  ...
                  isThinking ? "bg-blue-500" : "bg-emerald-500",
                )}
              />
              <span className="text-[10px] font-mono text-white/80 uppercase tracking-widest">
                {activeEmergencyLabel.toUpperCase()} // {status}
              </span>
            </div>

            <AnimatePresence>
              {(conversationState !== "assessment" || heartRate) && (
                <motion.div
                ...
                >
                  ...
                  <div
                    className={cn(
                      ...
                      vitalChecks.breathing
                        ? "bg-emerald-500/20 border-emerald-500 text-emerald-400"
                        : "bg-red-500/20 border-red-500 text-red-400",
                    )}
                  >
                    Breathing: {vitalChecks.breathing ? "YES" : "ASSESS"}
                  </div>
                  <div
                    className={cn(
                      "px-3 py-1 rounded-full text-[8px] font-bold uppercase tracking-tighter border",
                      vitalChecks.conscious
                        ? "bg-emerald-500/20 border-emerald-500 text-emerald-400"
                        : "bg-red-500/20 border-red-500 text-red-400",
                    )}
                  >
                    Conscious: {vitalChecks.conscious ? "YES" : "ASSESS"}
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
        ...
        {hapticPattern && (
          <div className="absolute bottom-32 left-1/2 -translate-x-1/2 flex items-center gap-2 px-4 py-2 bg-blue-500/20 backdrop-blur-xl border border-blue-500/40 rounded-full z-[100]">
            <motion.div
              ...
              transition={{
                duration: hapticPattern === "cpr" ? 0.55 : 2,
                repeat: Infinity,
              }}
        ...
        {showRadiologyTrainer && radiologyResult && (
          ...
          >
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                onClick={() => setShowRadiologyTrainer(false)}
            ...
            <div className="space-y-6">
              <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
                <div className="text-[8px] font-mono text-emerald-400 uppercase mb-1">
                  Diagnosis
                </div>
                <div className="text-xs font-bold text-white">
                  {radiologyResult.diagnosis}
                </div>
                <div className="mt-2 text-[8px] font-mono text-white/40">
                  Confidence: {(radiologyResult.confidence * 100).toFixed(1)}%
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                {radiologyResult.trainingInstructions.map(
                  (step: any, i: number) => (
                    <div
                      key={i}
                    ...
                    >
                      ...
                      {step.deviceOperation && (
                        <div className="mb-2 p-2 bg-blue-500/10 rounded-lg border border-blue-500/20">
                          <div className="text-[7px] font-mono text-blue-300 uppercase mb-1">
                            Device Operation
                          </div>
                          <div className="text-[9px] text-blue-100/80">
                            {step.deviceOperation}
                          </div>
                        </div>
                      )}
                      <div className="flex flex-wrap gap-1">
                        {step.anatomicalLandmarks.map(
                          (landmark: string, j: number) => (
                            <span
                              key={j}
                              className="px-2 py-0.5 bg-white/10 rounded text-[7px] text-white/60 font-mono"
                            >
                              {landmark}
                            </span>
                          ),
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  {radiologyResult.treatmentPlan.map((t: string, i: number) => (
                    <div
                      key={i}
                      className="text-[10px] text-white/60 flex gap-2"
                    >
              ...
              {radiologyResult.recoveryPlan &&
                radiologyResult.recoveryPlan.length > 0 && (
                  <div className="space-y-2">
                    <div className="text-[10px] font-mono text-indigo-400 uppercase">
                      Recovery Plan
                    </div>
                    <div className="space-y-1">
                      {radiologyResult.recoveryPlan.map(
                        (t: string, i: number) => (
                          <div
                            key={i}
                            className="text-[10px] text-white/60 flex gap-2"
              ...
              {radiologyResult.surgicalPlan &&
                radiologyResult.surgicalPlan.length > 0 && (
                  <div className="space-y-2">
                    <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-xl">
                      ...
                      <div className="space-y-2">
                        {radiologyResult.surgicalPlan.map(
                          (t: string, i: number) => (
                            <div
                              key={i}
                              className="text-[10px] text-white/80 flex gap-2"
                            >
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {radiologyResult.differentialDiagnosis.map(
                    (d: string, i: number) => (
                      <span
                        key={i}
                        className="px-2 py-1 bg-amber-500/10 border border-amber-500/20 rounded-lg text-[8px] text-amber-400"
                      >
                        {d}
                      </span>
                    ),
      ...
      {specialtyMode === "RADIOLOGY" &&
        radiologyResult?.gradCamSimulatedHeatmap && (
          <div className="absolute inset-0 pointer-events-none">
            {radiologyResult.gradCamSimulatedHeatmap.map(
              (point: any, i: number) => (
                <motion.div
                  key={i}
                  initial={{ opacity: 0 }}
      ...
      <AnimatePresence>
        {specialtyMode === "RADIOLOGY" &&
          radiologyWorkflowState !== "RESULTS" && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 20 }}
                ...
                {radiologyWorkflowState === "ASKING_SCAN_TYPE" && (
                  <div className="text-center space-y-4">
                    ...
                    <p className="text-white/40 text-sm">
                      Please state the scan type (e.g., "Brain MRI", "Chest CT")
                    </p>
                  </div>
                )}

                {radiologyWorkflowState === "PROVIDING_GUIDANCE" &&
                  deviceGuidance && (
                    <div className="space-y-6">
                      <div className="flex items-center justify-between">
                        <h2 className="text-xl font-bold text-white">
                          Device Guidance: {scanType}
                        </h2>
                        <div className="px-3 py-1 bg-blue-500/20 rounded-full text-[10px] text-blue-400 font-bold uppercase tracking-widest">
                          Technician Mode
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-6">
                          <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                              {deviceGuidance.safetyWarnings.map(
                                (warning, i) => (
                                  <div
                                    key={i}
                                    className="flex gap-2 p-2 bg-red-500/10 border border-red-500/20 rounded-lg text-[10px] text-red-400"
                                  >
                          ...
                          <div className="p-4 bg-white/5 border border-white/10 rounded-2xl">
                            ...
                            <div className="text-xs text-white font-mono">
                              {deviceGuidance.optimalSettings}
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <button
                        onClick={() => {
                          ...
                          speak(
                            "Guidance acknowledged. Please perform the scan and then capture or upload the resulting image for analysis.",
                          );
                        }}
                ...
                {radiologyWorkflowState === "AWAITING_SCAN" && (
                  <div className="text-center space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-white/40 text-sm">
                        Please point the camera at the scan or upload the
                        medical image file.
                      </p>
                    </div>
                            ...
                            if (file) {
                              ...
                              reader.onload = (ev) => {
                                ...
                                (async () => {
                                  setIsThinking(true);
                                  setRadiologyWorkflowState("ANALYZING");
                                  const result = await analyzeRadiology(
                                    base64,
                                    "File Upload",
                                    (window as any).isRadiologyTraining,
                                  );
            ...
            <div className="flex justify-between items-start mb-8">
              <div>
                ...
                <p className="text-xs text-white/40 font-mono uppercase tracking-widest mt-1">
                  Aegis Quantum v2.1 // MSR-001
                </p>
              </div>
              <button
                onClick={() => setShowMSR(false)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-8">
                <div className="p-6 bg-white/5 rounded-2xl border border-white/5">
                  <div className="text-[10px] font-mono text-emerald-400 uppercase mb-3">
                    Patient Status
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span
                        className={`text-3xl font-bold tracking-tighter ${severity === "critical" ? "text-red-500" : "text-emerald-500"}`}
                      >
                ...
                {predictiveAnalytics && (
                  <div className="p-6 bg-white/5 rounded-2xl border border-white/5">
                    <div className="text-[10px] font-mono text-blue-400 uppercase mb-3">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xl font-bold text-red-500">
                          {predictiveAnalytics.timeToCriticalityMin} minutes
                        </span>
                      </div>
                      <div className="space-y-2">
                        <span className="text-[8px] text-white/40 uppercase">
                          Organ System Risk Matrix
                        </span>
                        {predictiveAnalytics.organSystemRisk.map((risk, i) => (
                          <div key={i} className="flex items-center gap-3">
                            <span className="text-[10px] text-white/60 w-20 truncate">
                              {risk.system}
                            </span>
                ...
                {multiSpectralAnalysis && (
                  <div className="p-6 bg-white/5 rounded-2xl border border-white/5">
                    <div className="text-[10px] font-mono text-purple-400 uppercase mb-3">
                      Multi-Spectral Tissue Analysis
                    </div>
                    <div className="grid grid-cols-3 gap-4">
                      <div className="text-center">
                        ...
                        <div className="text-lg font-bold text-purple-400">
                          {(
                            multiSpectralAnalysis.hypoxiaDetection * 100
                          ).toFixed(1)}
                          %
                        </div>
                      </div>
                      <div className="text-center">
                        ...
                        <div className="text-lg font-bold text-red-400">
                          {(
                            multiSpectralAnalysis.internalBleedingProbability *
                            100
                          ).toFixed(1)}
                          %
                        </div>
                      ...
                      <div className="text-center">
                        ...
                        <div className="text-lg font-bold text-emerald-400">
                          {(
                            multiSpectralAnalysis.tissueViability * 100
                          ).toFixed(1)}
                          %
                        </div>
                      </div>
                ...
                {woundMetrics && (
                  <div className="p-6 bg-white/5 rounded-2xl border border-white/5">
                    <div className="text-[10px] font-mono text-amber-400 uppercase mb-3">
                      Surgical Precision Wound Metrics
                    </div>
                    <div className="grid grid-cols-3 gap-4">
                      <div className="flex flex-col">
                        <span className="text-[8px] text-white/40 uppercase">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-lg font-bold text-white">
                          {woundMetrics.surfaceAreaMm2}mm²
                        </span>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[8px] text-white/40 uppercase">
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-3">
                    {chatHistory.map((h, i) => (
                      <div
                        key={i}
                        className="text-sm text-white/70 leading-relaxed border-l-2 border-emerald-500/30 pl-4 py-1"
                      >
                        {h.content}
                      </div>
                    ))}
              ...
              <div className="flex flex-col items-center justify-center p-8 bg-white/5 rounded-2xl border border-white/5">
                ...
                <p className="text-xs text-white/40 text-center leading-relaxed max-w-[200px]">
                  Paramedics can scan this code to instantly import the
                  patient's vitals and treatment history.
                </p>
      ...
      <div className="h-32 bg-[#0A0A0A] border-t border-white/10 flex items-center justify-between px-8">
        <div className="flex items-center gap-4">
          <div className="px-4 py-3 rounded-2xl bg-red-500/10 border border-red-500/20">
            <span className="text-[10px] font-bold uppercase tracking-widest text-red-300">
              Speech-Driven Emergency Intake
            </span>
          </div>
        ...
        <div className="flex-1 max-w-sm h-12 bg-white/5 border border-white/10 rounded-2xl flex items-center px-6 mx-8">
          ...
          <span className="ml-4 text-[10px] font-mono text-white/40 uppercase tracking-widest truncate">
            {isMicOn ? "Emergency Voice Guidance Active" : "Voice Guidance Muted"}
          </span>
        </div>
