    ...
    try {
      ...
      if (ctx && videoRef.current && isActive) {
        ...
        if (responseText) {
          ...
          // Allow user to interrupt by setting isThinking to false before speaking
          setIsThinking(false);
      ...
      if (err.message === 'QUOTA_EXCEEDED') {
        ...
        // Stop the loop if quota exceeded
        return;
      } else {
        setStatus('Engine busy... retrying');
      }
    } finally {
      if (loopIdRef.current === currentLoopId) {
        setIsThinking(false);
        // Dynamic interval: 4s for paid key, 8s for free tier to avoid quota issues
        const interval = hasPaidKey ? 4000 : 8000;
        if (!userQuery && isActive) {
          loopTimeoutRef.current = setTimeout(() => performAILoop(), interval);
        }
      }
    }
  };
            ...
            <div className="space-y-4 w-full max-w-xs">
              ...
              <button
                onClick={() => {
                  setIsQuotaExceeded(false);
                  setStatus('Retrying...');
                  setTimeout(() => performAILoop(), 2000);
                }}
                className="w-full h-14 bg-emerald-500 text-black font-bold rounded-2xl flex items-center justify-center active:scale-95 transition-transform"
              >
                Retry Connection
              </button>
