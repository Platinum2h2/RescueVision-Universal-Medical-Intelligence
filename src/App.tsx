...
const StepRunner = lazy(() => import('./components/StepRunner.tsx').then(module => ({ default: module.StepRunner })));
const LiveInterface = lazy(() => import('./components/LiveInterface.tsx').then(module => ({ default: module.LiveInterface })));
const BenchmarkTest = lazy(() => import('./components/BenchmarkTest.tsx').then(module => ({ default: module.BenchmarkTest })));
const ProjectNarrative = lazy(() => import('./components/ProjectNarrative.tsx').then(module => ({ default: module.ProjectNarrative })));
const ResearchDashboard = lazy(() => import('./components/ResearchDashboard.tsx').then(module => ({ default: module.ResearchDashboard })));
...
export default function App() {
  ...
  const handleCloseupCapture = async (image: string) => {
    ...
    try {
      if (farImage) {
        const { analyzeInjury } = await import('./services/injuryAnalysis.ts');
  ...
  return (
    <div className="min-h-[100dvh] bg-[#050505] text-white font-sans selection:bg-emerald-500/30 flex justify-center">
      <div className="w-full max-w-md relative bg-black shadow-2xl shadow-emerald-500/5 min-h-[100dvh] flex flex-col">
        <AnimatePresence>
          {showDisclaimer && (
            <motion.div 
              initial={{ opacity: 0 }}
            ...
            >
              <div className="w-full max-w-sm bg-[#0A0A0A] border border-white/10 rounded-[2.5rem] p-6 md:p-8 shadow-2xl">
                <div className="w-12 h-12 rounded-2xl bg-red-500/10 border border-red-500/30 flex items-center justify-center mb-6">
                  <Shield className="w-6 h-6 text-red-400" />
                </div>
                <h2 className="text-xl md:text-2xl font-light tracking-tight mb-4 text-white">RescueVision: Safety Protocol</h2>
                <div className="space-y-4 text-white/40 text-xs md:text-sm leading-relaxed mb-8">
                  <p>
                ...
                <button 
                  ...
                  className="w-full py-4 bg-white text-black font-bold uppercase tracking-[0.2em] text-[10px] rounded-2xl hover:scale-[1.02] active:scale-[0.98] transition-all"
                >
                  I Understand & Accept
                </button>
              </div>
        ...
        <AnimatePresence mode="wait">
          {state === 'IDLE' && (
            ...
            <AnimatePresence>
              {showResearchData && (
                <motion.div 
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="fixed inset-0 z-[110] bg-black/95 backdrop-blur-3xl flex items-center justify-center p-0 md:p-6"
                >
                  <Suspense fallback={
                    <div className="w-full h-full md:h-auto md:max-w-4xl bg-[#0A0A0A] md:border border-white/10 md:rounded-[2rem] p-6 md:p-10 flex flex-col items-center justify-center gap-4">
                      <Loader2 className="w-10 h-10 text-emerald-500 animate-spin" />
                      <p className="text-white/40 font-mono text-[10px] uppercase tracking-widest">Loading Technical Report...</p>
                    </div>
                  }>
                    <ResearchDashboard onClose={() => setShowResearchData(false)}>
            ...
            <h1 className="text-4xl md:text-8xl font-light tracking-tighter mb-6">
              Rescue<span className="text-emerald-500 font-medium italic">Vision</span>
            </h1>
            
            <p className="max-w-xl text-white/40 text-xs md:text-lg leading-relaxed mb-12 font-light px-4">
            ...
            <div className="flex flex-col gap-4 w-full max-w-md px-4">
              <div className="grid grid-cols-2 gap-3 md:gap-4">
                <button 
                  onClick={handleStart}
                  className="group relative px-4 py-6 md:px-6 md:py-8 bg-white/5 border border-white/10 text-white font-bold uppercase tracking-[0.2em] text-[8px] md:text-[10px] rounded-2xl md:rounded-3xl overflow-hidden transition-all hover:scale-105 active:scale-95 hover:bg-white/10"
                >
                  <span className="relative z-10 flex flex-col items-center gap-2 md:gap-3">
                    <Shield className="w-5 h-5 md:w-6 md:h-6 text-emerald-500" />
                    <div className="flex flex-col">
                      <span>Emergency Scan</span>
                      <span className="text-[6px] md:text-[7px] opacity-40 font-normal">Step-by-Step</span>
                    </div>
                  </span>
                </button>
                ...
                >
                  <span className="relative z-10 flex flex-col items-center gap-2 md:gap-3">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-[6px] md:text-[7px] opacity-40 font-normal">Real-Time</span>
                    </div>
                  </span>
                </button>
                ...
                <button 
                  ...
                  className="group relative px-4 py-6 md:px-6 md:py-8 bg-blue-600/20 border border-blue-500/30 text-white font-bold uppercase tracking-[0.2em] text-[8px] md:text-[10px] rounded-2xl md:rounded-3xl overflow-hidden transition-all hover:scale-105 active:scale-95 shadow-[0_0_30px_rgba(37,99,235,0.1)]"
                >
                  <span className="relative z-10 flex flex-col items-center gap-2 md:gap-3">
                    <BarChart3 className="w-5 h-5 md:w-6 md:h-6 text-blue-400" />
                    <div className="flex flex-col">
                      <span>Rad Trainer</span>
                      <span className="text-[6px] md:text-[7px] opacity-60 font-normal">Interactive</span>
                    </div>
                  </span>
                </button>
                ...
                <button 
                  ...
                  className="group relative px-4 py-6 md:px-6 md:py-8 bg-emerald-600/20 border border-emerald-500/30 text-white font-bold uppercase tracking-[0.2em] text-[8px] md:text-[10px] rounded-2xl md:rounded-3xl overflow-hidden transition-all hover:scale-105 active:scale-95 shadow-[0_0_30px_rgba(5,150,105,0.1)]"
                >
                  <span className="relative z-10 flex flex-col items-center gap-2 md:gap-3">
                    <Camera className="w-5 h-5 md:w-6 md:h-6 text-emerald-400" />
                    <div className="flex flex-col">
                      <span>Rad Live</span>
                      <span className="text-[6px] md:text-[7px] opacity-60 font-normal">Diagnostic</span>
                    </div>
                  </span>
                </button>
              </div>
              ...
              <div className="flex flex-col gap-3">
                <label className="cursor-pointer px-6 py-4 bg-white/5 border border-white/10 text-white/40 font-mono uppercase tracking-[0.2em] text-[8px] md:text-[9px] rounded-2xl hover:bg-white/10 transition-colors text-center">
                ...
                <button 
                  ...
                  className="px-6 py-4 md:py-5 bg-emerald-500 text-black font-bold uppercase tracking-[0.2em] text-[8px] md:text-[10px] rounded-2xl hover:bg-emerald-400 transition-all flex items-center justify-center gap-3 shadow-[0_0_40px_rgba(16,185,129,0.2)]"
                >
                ...
                <button 
                  ...
                  className="px-6 py-4 md:py-5 bg-purple-600 text-white font-bold uppercase tracking-[0.2em] text-[8px] md:text-[10px] rounded-2xl hover:bg-purple-500 transition-all flex items-center justify-center gap-3 shadow-[0_0_40px_rgba(147,51,234,0.2)]"
                >
